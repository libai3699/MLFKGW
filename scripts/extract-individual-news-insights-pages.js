...
function resolveHref(href) {
  ...
  if (href.startsWith(INVESTOR_BASE)) {
...
function extractFilterLabel(html) {
  return decodeHtml(html.match(/<label for="tagselection">([\s\S]*?)<\/label>/)?.[1] || 'Team/Fund');
}
...
function extractPressReleaseArticles(html) {
  ...
  while ((match = rowRegex.exec(html)) !== null) {
    ...
    const links = [];
    const linkRegex = /<a href="([^"]+)"[^>]*title="([^"]*)"[^>]*>\s*([\s\S]*?)\s*<\/a>/g;
    let linkMatch;
...
function extractPressReleasesPage() {
  ...
  return {
    ...meta,
    filterLabel: extractFilterLabel(firstHtml),
    resultSummary: {
      from: 1,
...
existing.individualPressReleases = extractPressReleasesPage();
existing.individualInsights = extractInsightsPage();

writeFileSync(outputPath, `${JSON.stringify(existing, null, 2)}\n`);
console.log(
  `Updated ${outputPath} (individual press releases: ${existing.individualPressReleases.articles.length}, individual insights: ${existing.individualInsights.articles.length}, featured: ${existing.individualInsights.featured.length})`,
);
