...
const pagesDir = join(root, 'pages-src', 'individual');
const SITE_BASE = '/individual-investors';
...
function localHref(href) {
  ...
  return href.replace(/\.html(?=($|\?|#))/, '').replace(/^\/individual-investors/, SITE_BASE);
}
...
function extractMainContent(html) {
  ...
  if (!main) {
    main = extractBetween(html, 'id="page-wrapper"', 'id="footer-wrapper"');
  }
  if (!main) {
    main = html;
  }
...
function extractStaticPage(html) {
  ...
  const content = extractMainContent(contentArea || html);
  const preferHtml =
    (content.html || contentArea || '').includes('investment-returns-month-end') ||
    (content.html || contentArea || '').includes('performance-share-class-wrapper') ||
    (content.html || contentArea || '').includes('id="morningstar-ratings"') ||
    (content.html || contentArea || '').includes('id="lipper-rankings"') ||
    (content.html || contentArea || '').includes('id="holdings"') ||
    (content.html || contentArea || '').includes('id="fact-sheet"') ||
    (content.html || contentArea || '').includes('id="monthly-commentary"') ||
    (content.html || contentArea || '').includes('id="quarterly-commentary"') ||
    (content.html || contentArea || '').includes('id="philosophy-and-process"') ||
    (content.html || contentArea || '').includes('id="advanced-document-filtering"') ||
    (content.html || contentArea || '').includes('class="toggle-link"') ||
    (content.html || contentArea || '').includes('id="fund-select"') ||
    (content.html || contentArea || '').includes('id="mailing-schedule"') ||
    (content.html || contentArea || '').includes('id="historical-distributions"') ||
    ((content.html || contentArea || '').includes('accordion-container') &&
      (content.html || contentArea || '').includes('id="ss-2"')) ||
    ((content.html || contentArea || '').includes('id="team-id-key"') &&
      (content.html || contentArea || '').includes('class="table sortable"'));
...
function readPage(relativePath) {
  const candidates = [
    join(pagesDir, 'individual-investors', `${relativePath}.html`),
...
for (const fund of extracted.individualFunds) {
  ...
  if (!teamSlugsSeen.has(teamSlug)) {
    teamSlugsSeen.add(teamSlug);
    const teamHtml = readPage(`investments/${teamSlug}`);
    if (teamHtml) {
      teams[teamSlug] = extractTeamPage(teamHtml);
    }
  }
...
const staticPaths = [
  ...
  'terms-conditions',
  'privacy-policy',
  'proxy-policies-and-voting-record',
];
...
writeFileSync(
  join(root, 'src/data/investorIndividualInvestmentPages.json'),
  JSON.stringify({ teams, funds }, null, 2),
);
writeFileSync(
  join(root, 'src/data/investorIndividualContentPages.json'),
